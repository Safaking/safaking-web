...
import {
  ArrowLeft, LogOut, ShieldAlert, Truck, Package, Wallet, MapPin, ShieldCheck, PackageCheck, Clock, IndianRupee, AlertCircle,
} from 'lucide-react';
import { SupplierOrder, SupplierProfile, SupplierRates, deliverySetupMissing, rupees } from '@/lib/supplier';
...
export type SupplierTab = 'orders' | 'products' | 'payments' | 'delivery' | 'documents';

export const SUPPLIER_TABS: { id: SupplierTab; label: string; icon: typeof Package }[] = [
...
export const isSupplierTab = (value: string | null): value is SupplierTab =>
  SUPPLIER_TABS.some((tab) => tab.id === value);
...
/** Everything a supplier sees, given the data. The page loads it. */
export function SupplierDashboard({
  supplier, userId, orders, ordersLoading, rates, error, tab, onTabChange, onOrdersChanged, onSupplierSaved, onLogout,
}: {
  ...
  onSupplierSaved: (next: SupplierProfile) => void;
  onLogout: () => void;
}) {
  const stats = useMemo(() => {
          ...
          <div className="flex items-center justify-between h-20 gap-3">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="font-display font-black text-lg sm:text-xl text-royal-100 uppercase tracking-widest leading-none truncate">
                  Supplier Portal
                </h1>
            ...
            <div className="flex items-center gap-4 shrink-0">
              ...
              <button
                onClick={onLogout}
                className="flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-xs text-royal-100 font-bold uppercase tracking-wider transition-colors"
                aria-label="Log out"
              >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        ...
        {kyc !== 'verified' && (
          <Banner
            ...
            action={tab === 'documents' ? undefined : { label: 'Open Documents', onClick: () => onTabChange('documents') }}
          >
        ...
        {setupMissing && (
          <Banner
            ...
            action={tab === 'delivery' ? undefined : { label: 'Set delivery charges', onClick: () => onTabChange('delivery') }}
          >
        ...
        {/* In the Android app the bottom tabs switch these instead. */}
        <div className="sk-web-only flex gap-2 overflow-x-auto pb-2 mb-6 custom-scrollbar">
        ...
        {tab === 'orders' && (
          <SupplierOrders orders={orders} loading={ordersLoading} holdDays={rates.holdDays} onChanged={onOrdersChanged} />
        )}
        ...
        {tab === 'delivery' && <SupplierProfileForm supplier={supplier} onSaved={onSupplierSaved} />}
        {tab === 'documents' && <VerificationPanel ownerId={userId} subjectType="supplier" />}
      </main>
    </div>
  );
}
...
function Banner({
  ...
  return (
    <div className={`flex flex-wrap items-start gap-3 p-4 sm:p-5 mb-5 rounded-3xl border-2 shadow-sm ${colours}`}>
...
function Stat({
  ...
  return (
    <motion.div
      whileHover={{ y: -3 }}
      className="p-4 sm:p-5 rounded-3xl bg-white border border-amber-200/60 shadow-sm flex items-center gap-3 sm:gap-4"
    >
      <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center shrink-0 ${colours}`}>
        <Icon size={20} />
      </div>
      <div className="min-w-0">
        <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest leading-tight">{label}</p>
        <p className="text-xl sm:text-2xl font-display font-black text-maroon-950 mt-0.5 truncate">{value}</p>
      </div>
    </motion.div>
  );
}
